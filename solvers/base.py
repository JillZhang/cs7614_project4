...
def one_step_lookahead(env, discount_factor, state, v):
    ...
    for a in range(env.nA):
        if state == env.nS - 1:
            #print(env.P[state][a])
            A[a] = 0.0
        #if this is frozen lake problem
        elif 'C' not in env.desc:
            
            if  env.desc[int(state/ env.nrow), state % env.ncol] in b'H':
                A[a] = 0.0
            else:
            ...
            for prob, next_state, reward, done in env.P[state][a]:
                A[a] += prob * (reward + discount_factor * v[next_state])
#    if state == env.nS - 2:
#        print('{},A is {},current v is {}'.format(env.P[state],A,list(v)))
    return A
    ...
    def evaluate_policy(self, policy, discount_factor=1.0, max_steps=None, theta=0.00001):
        ...
        while max_steps is None or steps < max_steps:
            ...
            for s in range(env.nS):
                v = 0
                #it reaches the goal, return the values array
                if s == env.nS - 1:
                    V[s] = 0.0
                else:
                    # it is frozen lake prblem
                    try:
                        if  env.desc[int(s/ env.nrow), s % env.ncol] in b'H':
                            V[s] = 0.0
                            continue
